...
class TandiPay {
  constructor(config = {}) {
    ...
    this.httpClient = new _httpClient.default({
      baseURL: this.apiUrl,
  ...
  async initializePayment(options = {}) {
    ...
    if (!(0, _validator.validatePhone)(options.phone)) {
      throw new Error('Invalid phone number format. Must be 9-15 digits');
    }
    if (!(0, _validator.validateAmount)(options.amount)) {
      throw new Error('Valid amount greater than 0 is required');
    }
    if (!(0, _validator.validateCustomer)(options.customer)) {
    ...
    const reference = options.reference || (0, _reference.generateReference)(options.orderId);
  ...
  async isPaymentSuccessful(reference) {
    ...
    return successfulStatuses.includes((_payment$status = payment.status) === null || _payment$status === void 0 ? void 0 : _payment$status.toLowerCase());
  }
  ...
  handleError(error) {
    if (error.response) {
      ...
      const message = ((_error$response$data = error.response.data) === null || _error$response$data === void 0 ? void 0 : _error$response$data.message) || `API Error: ${error.response.status}`;
...
var _default = exports.default = TandiPay;